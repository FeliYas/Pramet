...
const columns = ['orden', 'path', 'titulo', 'descripcion'];

// Definición de rutas
const createRoute = route('servicios.store');
const updateRoute = (id) => route('servicios.update', { id });
const deleteRoute = (id) => route('servicios.destroy', { id });
...
<template>
    <div>
        ...
        <DataTable :columns="columns" :data="servicios" :createRoute="createRoute" :updateRoute="updateRoute"
            :deleteRoute="deleteRoute" recomendacion="44x44" />
    </div>
</template>
